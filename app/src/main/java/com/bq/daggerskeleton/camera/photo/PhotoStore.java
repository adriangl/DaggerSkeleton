...
public class PhotoStore extends Store<PhotoState> {
    ...
    @Inject
    public PhotoStore(CameraStore cameraStore, RotationStore rotationStore) {
    ...
    private void takePicture() {
        try {
            final CaptureRequest.Builder jpegRequest = cameraStore.state().cameraDevice.createCaptureRequest(CameraDevice.TEMPLATE_STILL_CAPTURE);
    ...
    private Integer getJpegOrientation() {
        CameraDescription cameraDescription = cameraStore.state().cameraDescription;

        boolean isFacingFront = cameraDescription.facing == CameraDescription.Facing.FRONT;

        int cameraDefaultOrientation = cameraDescription.sensorOrientation;
